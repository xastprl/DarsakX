...
from .UO_SingleShell_RayTrace import*
...
class rtrace: 
    def __init__(self,Radius,Focallength,Lengthpar,Lengthhyp,ShellThickness,Theta,Raydensity,xi=1,DetectorPosition=0,Error="no",Approx="no",Gp=None,Gh=None,dGp=None,dGh=None,NumCore=None, ParallelProcessingFor=None, SurfaceType='wo'):
        ...
        self.dl=np.sqrt(1/Raydensity)*10; self.raydensity=Raydensity; self.error=Error; self.approx=Approx; self.NumCore=NumCore; self.ParallelProcessingFor=ParallelProcessingFor; self.st=ShellThickness; self.surfacetype=SurfaceType
    ...
    def ray_data(self,i,j):
        ...
        elif self.surfacetype=="uo":
         ray_data=uo_ray_trace(self.r[j],self.x0[j],self.xi[j],self.theta[i],self.lp[j],self.lh[j],self.dl,self.detectorposition,self.Gp[j],self.Gh[j],self.dGp[j],self.dGh[j])### User Optics
        else: raise Exception("Enter Valid surface type: wo, co or uo")
    ...
    def rtrace(self):
        if self.ParallelProcessingFor is not None and self.ParallelProcessingFor != 'theta' and self.ParallelProcessingFor != 'shell':
            raise Exception("ParallelProcessingFor can be either 'theta' or 'shell'")
        elif (self.ParallelProcessingFor == 'theta' or self.ParallelProcessingFor== 'shell') and self.NumCore is None:
            raise Exception("NumCore is required for parallel processing.")
        elif self.ParallelProcessingFor is None and self.NumCore is not None:
            raise Exception("ParallelProcessingFor is required for parallel processing.")
    
        ParallelProcessingFor=self.ParallelProcessingFor
        ...
        if self.NumCore is None:
           ...
           for i in range(len(self.theta)):
        ...
        elif ParallelProcessingFor=='shell':
            ...
            pool.close()
            pool.join()
        elif ParallelProcessingFor=='theta':
            pool = mp.Pool(processes=self.NumCore)
            ray_data_alltheta_allr=[]
            for j in range(len(self.r)):
                param=[]
                for i in range(len(self.theta)):
                    param.append([i,j])
                ray_data_oneradius=pool.starmap(self.ray_data,param)
                ray_data_alltheta_allr.append(ray_data_oneradius)
            pool.close()
            pool.join()
            ray_data_alltheta_allr=list(map(list, zip(*ray_data_alltheta_allr)))
